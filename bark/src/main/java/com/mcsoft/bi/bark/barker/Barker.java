            ...
            try {
                ...
                if (doNotice) {
                    ...
                    String messageBuilder = config.getSymbol() + "最近" +
                            config.getSeconds() + "秒内价格变动达到" + percent.setScale(2, RoundingMode.HALF_UP) + "%，" +
                            "达到阈值：" + config.getPercent() + "%。" +
                            "当前价格：" + end.getPrice() + "，" +
                            "上一极端价格：" + start.getPrice();
                    dingBotApi.sendMessageToBiGroup(messageBuilder);
                    // 如发出了通知，则至少等待一分钟后继续通知
                    loopWaitSeconds = Math.max(config.getSeconds() / 3, 60);
                }
            } catch (Exception ex) {
                log.error(getThreadName() + "出现异常", ex);
                // 出现异常则等待10秒
                loopWaitSeconds = 10;
            } finally {
                try {
                    Thread.sleep(TimeUnit.SECONDS.toMillis(loopWaitSeconds));
                } catch (InterruptedException ignore) {
                }
            }
        }
        log.info("barker：" + config + "终止完成");
    }
